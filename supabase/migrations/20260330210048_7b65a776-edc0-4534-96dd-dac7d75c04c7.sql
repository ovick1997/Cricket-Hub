...
BEGIN
  ...
  -- Get team1 score from whichever innings they batted
  ...
  -- Get team2 score from whichever innings they batted
...
-- Fix existing null score for match 4e89a6cf
UPDATE match_summaries 
SET team1_score = (
  SELECT total_runs || '/' || total_wickets || ' (' || total_overs || ' ov)' 
  FROM innings i JOIN matches m ON m.id = i.match_id 
  WHERE i.match_id = match_summaries.match_id AND i.batting_team_id = m.team1_id
  LIMIT 1
)
...
SET team2_score = (
  SELECT total_runs || '/' || total_wickets || ' (' || total_overs || ' ov)' 
  FROM innings i JOIN matches m ON m.id = i.match_id 
  WHERE i.match_id = match_summaries.match_id AND i.batting_team_id = m.team2_id
  LIMIT 1
)
WHERE team2_score IS NULL;